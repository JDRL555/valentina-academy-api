...
export default function DeleteModal({ showModal, setShowModal, ingredientId }) {

  const [deleting, setDeleting] = useState(false)
  
  const onDelete = async () => {
    setDeleting(true)
    await fetchToApi(`${BACKEND_ROUTES.ingredients}/${ingredientId}`, {
      method: "DELETE",
      headers: {
    ...
    <Modal showModal={showModal} setShowModal={setShowModal}>
      {
        ...
        <div className='admin_deleting_container'>
          ...
          <h1>Eliminando ingrediente...</h1>
        </div>
        :
        <>
          <h1>Estas seguro de eliminar el ingrediente?</h1>
