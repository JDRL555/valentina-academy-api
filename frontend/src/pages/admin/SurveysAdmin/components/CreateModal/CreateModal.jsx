...
export default function CreateModal({ 
  ...
  const onInputChange = e => {
    setNewSurvey({ ...newSurvey, [e.target.id]: e.target.value })
  }

  const onQuestionChange = e => {
    if(!newSurvey.questions[e.target.id]) {
      newSurvey.questions.push({ 
        question: e.target.value, 
        answers: [] 
      })
    } else {
      newSurvey.questions[e.target.id] = { 
        ...newSurvey.questions[e.target.id], 
        question: e.target.value 
      }
    }
  ...
  const onAnswerChange = (question_index, index, value, is_correct) => {
    if(!newSurvey.questions[question_index].answers[index]) {
      newSurvey.questions[question_index].answers.push({
        answer: value,
        is_correct
      })
    } else {
      newSurvey.questions[question_index].answers[index] = {
        ...newSurvey.questions[question_index].answers[index],
        answer: value
      }
    }
  ...
  const onIsTrueChange = (question_index, index) => {
    newSurvey.questions[question_index].answers = newSurvey.questions[question_index].answers.map((answer, answerIndex) => {
  ...
  const onCreateSurvey = async e => {
    ...
    const questionPromises = newSurvey.questions.map(async question => {
      let answersArr = []
    ...
    const surveysResponse = await fetchToApi(BACKEND_ROUTES.surveys, {
      method: "POST",
      body: JSON.stringify({
        ...newSurvey, question_id: questionsArr
      }),
      headers: {
        'Content-type': "application/json"
      }
